...
if (App::runningInConsole() || App::runningUnitTests()) {
    // If running in console or tests, load a dummy bot or handle appropriately
    // This prevents database errors during artisan commands or testing
    $bots['default'] = [
    ...
    try {
        foreach (TelegramBot::where('is_active', true)->get() as $bot) {
            $bots[$bot->name] = [
                'token' => $bot->token,
        ...
        Log::error("Error loading Telegram bots from database: " . $e->getMessage());
    }
}
...
return [
    ...
    */
    ...
    |
    */
    'default' => array_key_first($bots) ?: 'mybot',
