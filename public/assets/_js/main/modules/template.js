...
export default class Template {
   ...
   */
  constructor(options) {
    this.onLoad(() => this._uiInit(options));
  }
  ...
  _uiInit(options) {
    ...
    this._lSidebarScrollCon =
      this._lSidebar && this._lSidebar.querySelector(".js-sidebar-scroll");
  ...
  _uiHandleHeader() {
    ...
    if (
      self._lPage.classList.contains("page-header-glass") &&
      self._lPage.classList.contains("page-header-fixed")
    ) {
   ...
   *
   */
  _uiHandleDarkMode(mode) {
    let self = this;
    let rememberDarkMode = !self._lHtml.classList.contains(
      "dark-custom-defined"
    );
    ...
    if (["on", "off", "system"].includes(mode)) {
      if (mode === "on") {
        ...
        if (rememberDarkMode) {
          localStorage.setItem("dashmixDarkMode", "on");
        }
        ...
        // Set dark mode icon
        if (darkModeIcon) {
          darkModeIcon.className = "fa fa-fw fa-moon";
        }
      } else if (mode === "off") {
        self._lHtml.classList.remove("dark");
        self._darkMode = "off";
        ...
        // Set dark mode icon
        if (darkModeIcon) {
          darkModeIcon.className = "far fa-fw fa-moon";
        }
      } else if (mode === "system") {
        ...
        // Save dark mode preference
        if (rememberDarkMode) {
          localStorage.setItem("dashmixDarkMode", "system");
        }
      ...
      // If the preference is saved before, else use the default
      if (rememberDarkMode) {
        darkMode = localStorage.getItem("dashmixDarkMode") || self._darkMode;
      } else {
  ...
  _uiHandleTheme() {
    ...
    let rememberTheme = this._lHtml.classList.contains("remember-theme");

    // If remember theme is enabled
    if (rememberTheme) {
      let themeName = localStorage.getItem("dashmixColorTheme") || false;
      ...
      localStorage.removeItem("dashmixColorTheme");
    }
    ...
    document
      .querySelectorAll(
        '[data-toggle="theme"][data-theme="' +
          (themeEl ? themeEl.getAttribute("href") : "default") +
          '"]'
      )
      .forEach((link) => {
    ...
    document.querySelectorAll('[data-toggle="theme"]').forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        if (rememberTheme) {
          localStorage.setItem("dashmixColorTheme", themeName);
        }
      });
    });
  }
  ...
  _uiApiLayout(mode = "init") {
    ...
    let layoutAPI = {
      ...
      dark_mode_toggle: () => {
        if (self._lHtml.classList.contains("dark")) {
  ...
  _uiApiBlocks(mode = "init", block = false) {
        ...
        document
          ...
          .forEach((btn) => {
            btn.innerHTML =
              ...
              iconBase +
              " " +
              (btn.closest(".block").classList.contains("block-mode-fullscreen")
        ...
        document
          ...
          .forEach((btn) => {
            btn.innerHTML =
              ...
              iconBase +
              " " +
              (btn.closest(".block").classList.contains("block-mode-hidden")
                ? iconContentActive
                : iconContent) +
              '"></i>';
          });
        ...
        if (btnFullscreen) {
          if (elBlock.classList.contains("block-mode-fullscreen")) {
            btnFullscreen &&
              btnFullscreen
                .querySelector("i")
                .classList.replace(iconFullscreen, iconFullscreenActive);
          } else {
            btnFullscreen &&
              btnFullscreen
                .querySelector("i")
                .classList.replace(iconFullscreenActive, iconFullscreen);
          }
        }
      },
      fullscreen_on: () => {
      ...
      content_toggle: () => {
        ...
        if (btnContentToggle) {
          if (elBlock.classList.contains("block-mode-hidden")) {
            btnContentToggle
              .querySelector("i")
              .classList.replace(iconContent, iconContentActive);
          } else {
            btnContentToggle
              .querySelector("i")
              .classList.replace(iconContentActive, iconContent);
          }
        }
      },
      content_hide: () => {
    ...
    if (mode === "init") {
      ...
      // Get block element
      elBlock =
        block instanceof Element ? block : document.querySelector(`${block}`);

      // If element exists, procceed with block functionality
      if (elBlock) {
        // Get block option buttons if exist (need them to update their icons)
        btnFullscreen = elBlock.querySelector(
          '[data-toggle="block-option"][data-action="fullscreen_toggle"]'
        );
        btnContentToggle = elBlock.querySelector(
          '[data-toggle="block-option"][data-action="content_toggle"]'
        );
